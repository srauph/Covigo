...
from appointments.utils import cancel_appointments, delete_availabilities
...
def book_appointments(request):
    ...
    if request.method == 'POST' and request.POST.get('Book Appointment'):
        ...
        # books a single existing appointment by adding the patient's id to the appointment's patient_id column
    ...
    if request.method == 'POST' and request.POST.get('Book Selected Appointments'):
        ...
        # books all selected existing appointments by adding the patient's id to the appointment's patient_id column
        for booking_id in booking_ids:
            booking = Appointment.objects.get(id=booking_id)
            booking.patient = request.user
            booking.save()

        # success message to show to the user if all selected existing appointments were booked successfully
